...
| **أزرار لصق 📋** | على 6 حقول: العنوان، Slug، الكاتب، الملخص، الوسوم، الصورة |
| **إنذار جدولة** | يظهر تلقائياً عند تحديد تاريخ مستقبلي مع المدة المتبقية |
...
| **Git / نشر** | مزامنة (Pull مع rebase تلقائي) + commit + push |
| **حل التعارض** | Modal تفاعلي عند تعارض الملفات — نقرتان بدل طرفية |
| **المستودعات** | إضافة GitHub/GitLab/Codeberg + دفع لمستودعات متعددة |
| **الأمان** | كلمة مرور (SHA-256) + تأكيد للإجراءات الحساسة + مدة جلسة قابلة للضبط |
...
> **مزامنة تلقائية:** عند إنشاء القسم من اللوحة المحلية، يُحدَّث `cms/config.yml` تلقائياً ليظهر في Decap CMS (في `category` و `also_in` لكلتا اللغتين). لا حاجة لتعديل يدوي.
...
| الموقف | الإجراء |
|---|---|
| تعديل من Decap CMS | `start.sh` يسحب تلقائياً عند التشغيل |
| اللوحة المحلية تُظهر تنبيه أصفر | اضغط **مزامنة (Pull)** — rebase تلقائي |
| تباعد commits محلية وبعيدة | rebase --autostash تلقائي بدون تدخل |
| تعارض حقيقي في نفس السطر | Modal **حل التعارض** — نقرتان لكل ملف |
...
اضبط تاريخاً مستقبلياً لأي مقال:
- اللوحة تُظهر إنذاراً ذهبياً بمدة الانتظار
- على الموقع: البطاقة تحمل شارة **"سينشر قريباً 🕐"**
- صفحة المقال: ترحيب جميل بعدّاد تنازلي حي بدل شاشة 404
- يتحوّل المحتوى تلقائياً للعرض الكامل عند الموعد

...
## ⚠️ المنشور متأخّر عن المصدر

الكود في `src/` يحوي **تعديل/حذف الأقسام مع مهلة التراجع**، لكنه **لم يُنشر بعد** —
اللوحة الحيّة ما زالت تعمل بالنسخة السابقة. اللوحة المحلية (`admin/`) تحوي كل شيء.

السبب: تعذّر الوصول إلى حساب Cloudflare (كان الدخول عبر GitHub ثم تعطّل، واستعادة
كلمة المرور ببريد مختلف أنشأت حساباً جديداً فارغاً بدل استعادة الأصلي).

الحساب الصحيح: `account_id = f03637899e8ad8c55b649300337eeaa8` · نطاق `gnutux-arabic`
· Worker باسم `gt-newstech-admin`. التفاصيل وخطوات الاستعادة في `CLAUDE.md`
قسم «Where things stand».

البناء نفسه سليم: `npx wrangler deploy --dry-run` ينجح (71.59 KiB).
بعد استعادة الحساب: `export CLOUDFLARE_API_TOKEN=… && npx wrangler deploy`.
...
| رفع/حذف صورة | ✅ |
| إنشاء قسم جديد | ✅ |
| تعديل/حذف قسم + مهلة تراجع 30 ث | ✅ في المصدر — ⏳ ينتظر النشر |

...
- **The extension is taken from the file's bytes, not its name.** `detectExt()` in `admin/server.js` runs `sharp().metadata()` and maps the real format to an extension (sharp reports AVIF as `heif`). Without this, an AVIF served from a URL ending in `.png` lands in the library as `.png`: browsers still render it (they sniff content) but every sharp run fails on it. That is exactly how `claudevscodex.png` — a real AVIF, and truncated — sat in the library erroring on every CI run.
- **Batch script** for existing images: `cd scripts && npm run optimize` (or `:dry` / `:new` / `:avif`). First full run on the library (96 images): **40 MB → 26 MB (-45%)**, plus 94 WebP companions totaling 3.92 MB. Heavy screenshots reduced 70-78% via resize to 1600px.
- **`--new` must never re-process already-compressed formats.** Its skip check keys off the presence of a `.webp` companion — but avif/webp masters never get one, so they matched nothing and were re-encoded lossily on *every* CI run (cumulative quality loss). `--new` now returns early for any non-`MASTER_EXTS` file.

**AVIF (`--avif`, opt-in):**
- `npm run optimize:avif` generates `.avif` companions next to `.webp` for every JPG/PNG. Measured on this library: AVIF is **22-38% smaller than the WebP companion**, but encoding costs **3-17 s per image** (vs ~0.3 s for WebP) — hence opt-in rather than wired into `generate-webp.yml`.
- The generator discards an AVIF companion that is not smaller than the WebP one, since `<source type="image/avif">` comes first and the browser would otherwise pick the larger file.
- To roll it out across the library and keep it automatic afterwards, run `npm run optimize:avif` once and add `--avif` to the `generate-webp.yml` step.

**Image markup — `_includes/responsive-image.html`:**
- Single include used by both `_includes/article-card.html` and `_layouts/post.html`; it replaced the near-identical blocks that had drifted in each. Emits `<picture>` with AVIF → WebP → original, each `<source>` only when that companion actually exists.
- **Companion existence and dimensions both come from `_data/images-index.json`**, via the `ar_map` / `en_map` hash (`name → {w,h}`). The templates used to scan `site.static_files` for every image on every page; the hash lookup is O(1) and cut a full build from ~33 s to ~26 s even after adding a second format to check.
- Emits `width`/`height` on every `<img>` so the browser reserves the correct box before the image loads (no layout shift) and knows the aspect ratio. `scripts/build-images-index.js` records them with sharp — which is why **`build-images-index.yml` now needs an `npm ci` step**; the script previously used no dependencies.
- Handles external URLs (Decap CMS "Insert from URL"): no companions, no dimensions, plain `<img>`. The old inline code prepended the baseurl to them, producing `/GT-NEWSTECH/assets/images/ar/https://…`.
- `<img>` keeps the `onerror` fallback guarded by `data-fallen=1` against an infinite loop if the fallback icon also fails.

**Images are never cropped — `contain`, not `cover`:**
- Cards use a fixed `aspect-ratio: 16/9` box (keeps the grid even) with `object-fit: contain`, so any ratio shows in full. Measured across the library: 0 cropped, median empty bar area 1.6%, and 9 of 25 cards land exactly on 16:9 with no bars at all.
- The hero is `height: auto` — the box takes the image's own ratio, so there is neither crop nor letterbox. `max-height` is deliberately a large **px** value (1200px), not a `vh` one: any `vh` cap makes the box shorter than the image's natural ratio at full width, and `contain` then adds side bars to ordinary landscape photos. At the 1160px content width, 1200px only ever engages for portrait uploads.
- Bars, when they appear, use `--img-bg`. Card hover is a `brightness` shift, **not** `transform: scale` — scaling inside `overflow: hidden` crops the edges, which is the thing being fixed.
- The mobile rule `img[width] { width:100% !important; height:auto !important }` had to go: now that every `<img>` carries a `width` attribute it matched all of them and overrode the aspect-ratio boxes. It is scoped to `.post-body` / `.static-page-body` content images and no longer uses `!important`.
